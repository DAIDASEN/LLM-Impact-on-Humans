...
from llm_pipeline.prompts import SCREEN_SYSTEM, screen_user_prompt
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage 2: empirical screening + cluster assignment.")
    ap.add_argument("--csv", default="papers.csv")
    ap.add_argument("--papers_dir", default="papers")
    ap.add_argument("--out", default="llm_outputs/screen_cluster.jsonl")
    ap.add_argument("--resume", action="store_true", help="Skip filenames already present in --out")
    ap.add_argument("--only_stage1_match", action="store_true", help="Only process rows where stage1_is_paper && stage1_is_match is True in the CSV")
    ap.add_argument("--csv_out", default="", help="Write an enriched CSV with stage2 columns")
    ...
    if args.only_stage1_match and "stage1_is_paper" in df.columns and "stage1_is_match" in df.columns:
        df = df[(df["stage1_is_paper"] == True) & (df["stage1_is_match"] == True)].reset_index(drop=True)
    if args.start:
    ...
    with out_path.open(write_mode, encoding="utf-8") as f:
        for _, r in tqdm(df.iterrows(), total=len(df)):
            ...
            try:
                raw = call_llm(system_prompt=SCREEN_SYSTEM, user_prompt=user_prompt, reasoning_effort="minimal")
    ...
    # Optionally write back to CSV (stage2_cluster_id / stage2_custom_summary)
    if args.csv_out or args.inplace:
        result_map = load_result_map_by_filename(out_path)
        df2 = pd.read_csv(args.csv)
        df2["stage2_cluster_id"] = df2["filename"].map(lambda x: result_map.get(str(x), {}).get("cluster_id") if str(x) in result_map else None)
        df2["stage2_custom_summary"] = df2["filename"].map(lambda x: result_map.get(str(x), {}).get("custom_summary") if str(x) in result_map else None)
        ...
        if args.inplace:
            ...
            out_csv = Path(args.csv_out) if args.csv_out else Path("papers_stage2.csv")
